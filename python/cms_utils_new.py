...
if 'condor' in os.getcwd():
    uploadDir = 'dask-scratch-space/zjet/'
else:
    uploadDir = ''
...
"RelativeStatEC","RelativeStatFSR","RelativeStatHF","SinglePionECAL","SinglePionHCAL","TimePtEta"]
    ...
    #For MC
        ext.add_weight_sets([
            '* * '+uploadDir+'correctionFiles/JEC/{0}/{0}_L1FastJet_AK8PFPuppi.jec.txt'.format(jec_tag),
            '* * '+uploadDir+'correctionFiles/JEC/{0}/{0}_L2Relative_AK8PFPuppi.jec.txt'.format(jec_tag),
            '* * '+uploadDir+'correctionFiles/JEC/{0}/{0}_L3Absolute_AK8PFPuppi.jec.txt'.format(jec_tag),
            '* * '+uploadDir+'correctionFiles/JEC/{0}/{0}_UncertaintySources_AK8PFPuppi.junc.txt'.format(jec_tag),
            '* * '+uploadDir+'correctionFiles/JEC/{0}/{0}_Uncertainty_AK8PFPuppi.junc.txt'.format(jec_tag),
        ])
        ...
        if jer_tag:
            ...
            ext.add_weight_sets([
            '* * '+uploadDir+'correctionFiles/JER/{0}/{0}_PtResolution_AK8PFPuppi.jr.txt'.format(jer_tag),
            '* * '+uploadDir+'correctionFiles/JER/{0}/{0}_SF_AK8PFPuppi.jersf.txt'.format(jer_tag)])
        ...
        for run, tag in jec_tag_data.items():
            if not (tag in tags_done):
                ext.add_weight_sets([
                '* * '+uploadDir+'correctionFiles/JEC/{0}/{0}_L1FastJet_AK8PFPuppi.jec.txt'.format(tag),
                '* * '+uploadDir+'correctionFiles/JEC/{0}/{0}_L2Relative_AK8PFPuppi.jec.txt'.format(tag),
                '* * '+uploadDir+'correctionFiles/JEC/{0}/{0}_L3Absolute_AK8PFPuppi.jec.txt'.format(tag),
                '* * '+uploadDir+'correctionFiles/JEC/{0}/{0}_L2L3Residual_AK8PFPuppi.jec.txt'.format(tag),
                ])
                tags_done += [tag]
    ext.finalize()
...
def GetPUSF(IOV, nTrueInt, var='nominal'):
    ...
    fname = uploadDir+"correctionFiles/POG/LUM/" + corrlib_namemap[IOV] + "/puWeights.json.gz"
    hname = {
...
def GetEleSF(IOV, wp, eta, pt, var = ""):
    ...
    fname = uploadDir+"correctionFiles/POG/EGM/" + corrlib_namemap[IOV] + "/electron.json.gz"
    year = {
        "2016APV" : "2016preVFP",
        "2016"    : "2016postVFP",
        "2017"    : "2017",
...
def GetMuonSF(IOV, corrset, abseta, pt, var="sf"):
    ...
    if 'APV' in IOV:
        tag = '2016_preVFP'
    fname = uploadDir+"correctionFiles/muonSF/UL"+IOV+"/ScaleFactors_Muon_highPt_"+corrset+"_"+tag+"_schemaV2.json"
...
def getLumiMaskRun2():
    ...
    golden_json_path_2016 = uploadDir+"correctionFiles/goldenJsons/Cert_271036-284044_13TeV_Legacy2016_Collisions16_JSON.txt"
    golden_json_path_2017 = uploadDir+"correctionFiles/goldenJsons/Cert_294927-306462_13TeV_UL2017_Collisions17_GoldenJSON.txt"
    golden_json_path_2018 = uploadDir+"correctionFiles/goldenJsons/Cert_314472-325175_13TeV_Legacy2018_Collisions18_JSON.txt"
