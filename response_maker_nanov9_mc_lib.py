...
#         #####################################
        ...
        event = events[ np.abs(vals) < 2 ]
        self.hists["cutflow"][dataset]["weights cut"] += len(events)

        
        events = events[events.PV.npvsGood > 0]
        self.hists["cutflow"][dataset]["npv_v2"] += len(events) #good npv cut

        events = events[ak.sum( (events.GenJetAK8.pt > 136.) & (np.abs(events.GenJetAK8.eta) < 2.5), axis=1 ) >= 1]
        events = events[ak.sum( (events.FatJet.pt > 170.) & (np.abs(events.FatJet.eta) < 2.5), axis=1 ) >= 1]
